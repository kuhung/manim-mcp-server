...
从 v0.2.0 开始，Manim MCP Server 支持自定义脚本存放位置和视频输出位置，让您更好地组织和管理生成的文件。

### 🎯 功能特性

- **自定义脚本目录**: 指定 Manim 脚本文件的保存位置
- **自定义输出目录**: 指定生成视频的输出位置  
- **自定义脚本名称**: 自定义脚本文件名（不含扩展名）
- **路径展开**: 支持 `~` 家目录符号和相对路径
- **自动创建**: 指定的目录不存在时会自动创建
...
```json
{
  ...
  "arguments": {
    "search_dir": "~/Documents/manim_videos"
  }
}
```
...
| 参数 | 类型 | 必需 | 说明 |
|------|------|------|------|
| `code` | string | ✅ | Manim Python 代码 |
| `script_dir` | string | ❌ | 脚本文件保存目录 |
| `output_dir` | string | ❌ | 视频输出目录 |
| `script_name` | string | ❌ | 脚本文件名（默认: "scene"） |

### 💡 使用技巧

1. **家目录符号**: 使用 `~` 表示用户家目录，如 `~/Documents/manim_projects`
2. **相对路径**: 支持相对路径，如 `./my_scripts` 或 `../shared_videos`
3. **绝对路径**: 支持完整的绝对路径，如 `/Users/username/Projects/manim`
4. **自动创建**: 指定的目录不存在时会自动创建，无需手动创建
5. **跨平台**: 路径格式会自动适配不同操作系统
...
**在 MCP 客户端中的对话**:
```
用户：请创建一个关于微积分的动画，脚本保存到 ~/Documents/manim_projects/math_animations/scripts，视频输出到 ~/Documents/manim_projects/math_animations/videos

Claude：我来为您创建微积分动画，并按您指定的路径保存...
```
